...
object jugando {
	...
	method activarse(){
		...
		keyboard.s().onPressDo { if (juego.estadoActual().equals(self)) marcoSelector.tomaFicha(marcoSelector.position()) }
		keyboard.m().onPressDo { if (juego.estadoActual().equals(self)) marcoSelector.moveFicha(marcoSelector.position()) }
		keyboard.p().onPressDo { if (juego.estadoActual().equals(self)) juego.pausar() }
		keyboard.enter().onPressDo { if (juego.estadoActual().equals(pausado)) juego.reanudarJuego() }
		keyboard.left().onPressDo { if (juego.estadoActual().equals(self)) marcoSelector.movete(marcoSelector.position().left(1)) }
		keyboard.right().onPressDo { if (juego.estadoActual().equals(self)) marcoSelector.movete(marcoSelector.position().right(1)) }
		keyboard.up().onPressDo { if (juego.estadoActual().equals(self)) marcoSelector.movete(marcoSelector.position().up(1))} 
		keyboard.down().onPressDo { if (juego.estadoActual().equals(self)) marcoSelector.movete(marcoSelector.position().down(1))} 
...
object menuPrincipal {
	...
	var opcionActual = opcionJugar

	method navegar(){
		game.addVisual(opcionActual)
		keyboard.down().onPressDo {  if (juego.estadoActual().equals(self)){ game.sound("menuSonido.ogg") opcionAnterior = opcionActual opcionActual = opcionActual.opcionDeAbajo() game.addVisual(opcionActual) if ( opcionAnterior != null ) game.removeVisual(opcionAnterior)  }}
		keyboard.up().onPressDo {  if (juego.estadoActual().equals(self)){ game.sound("menuSonido.ogg") opcionAnterior = opcionActual opcionActual = opcionActual.opcionDeArriba() game.addVisual(opcionActual) if ( opcionAnterior != null ) game.removeVisual(opcionAnterior)  }}
		keyboard.enter().onPressDo { if (juego.estadoActual().equals(self)) game.sound("entrarOpcion.ogg") opcionActual.accion() }
	}
